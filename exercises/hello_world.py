# List of team members' names
team_names = ["Kira Espinoza", "Júlia Monteiro", "Camila Licona", "Karla Serna"]
print("Hello World and welcome to the team!")
print("Course: Algorithms and Data Analysis")
# Loop through each name and print a welcome message
for name in team_names:
    print("Welcome,", name)
# len() counts how many names are in the list
print("We are", len(team_names), "students ready to learn Python!")
